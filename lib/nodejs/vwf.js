...
function HandleParsableRequest( request, response ) {
    ...
    if ( ( request.url[ request.url.length - 1 ] != "/" ) && ( parsedRequest[ 'private_path' ] == undefined ) ) {
        if ( ( request.headers['user-agent'] ) && ( request.headers['user-agent'].indexOf("MSIE 8.0" ) > -1 ) ) {
            ...
            return servehandler.Component( request, response, helpers.JoinPath( process.env.HOME + "/.vwf/public/", parsedRequest[ 'public_path' ], parsedRequest[ 'application' ] ) );
        }
        else {
...
function HandleProxyRequest( request, response ) {
    ...
    if ( ( segments.length > 0 ) && ( segments[ 0 ] == "proxy" ) ) {
        if ( servehandler.File( request, response, helpers.JoinPath( process.env.HOME + "/.vwf/support/", updatedURL ) ) ) {
            return true;
        }
        if ( servehandler.Component( request, response, helpers.JoinPath( process.env.HOME + "/.vwf/support/", updatedURL ) ) ) {
            return true;
        }
    }
    return false;
...
function HandleFileRequest( request, response ) {
    ...
    return servehandler.File( request, response, helpers.JoinPath( process.env.HOME + "/.vwf/public/", updatedURL ) );
}

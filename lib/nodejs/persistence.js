...
function GenerateSaveObject( request, public_path, application, instance, save_name ) {
    ...
    var metadata = storage.SaveMetadata( public_path, application, instance, save_name );
    if ( metadata ) {
    ...
    if ( request.method == "GET" ) {
        ...
        if ( segments.length == 1 ) {
            ...
            if ( parsedRequest[ 'instance' ] ) {
                ...
                var save_name_list = storage.InstanceSaveStates( parsedRequest[ 'public_path' ], parsedRequest[ 'application' ], parsedRequest[ 'instance' ] );
                var save_objects = [ ];
                ...
                var save_hash = storage.ApplicationSaveStates( parsedRequest[ 'public_path' ], parsedRequest[ 'application' ] );
                var instance_hash = { };

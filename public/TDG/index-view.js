    ...
    if ( kineticClass !== undefined ) {
        ...
        switch ( childPrototypeID ) {
            ...
            case "tdgLayer.vwf":
                ...
                kineticNode.isTDGLayer = true;
                addLayerDropDown( kineticNode, vwf_view.kernel.client() );
                break;
        }
        ...
        switch ( childPrototypeID ) {
            ...
            case "participants.vwf":
                _participants[ childID ] = _participants[ childName ] = {
                    "ID": childID,
                    "parentID": nodeID,
                    "name": childName,
                    "prototypeID": childPrototypeID,
...
vwf_view.deletedNode = function( nodeID ) {
    ...
    if ( kineticComponents[ nodeID ] ) {
        ...
        var node = kineticComponents[ nodeID ];
        if ( node.isTDGLayer ) {
            setActiveLayer( "baseLayer" );
            removeLayerDropDown( kineticNode );    
        }
...
vwf_view.satProperty = function( nodeID, propertyName, propertyValue ) {
    ...
    if ( !require("vwf/utility").validPropertyValue( propertyValue ) ) {
        return;
    }
    ...
    switch ( propertyName ) {
        ...
        case "visibleName":
            if ( _participants[ nodeID ] !== undefined ) {
                _participants[ nodeID ].visibleName = propertyValue;    
            }
            break;

        case "instructor":
            if ( _participants[ nodeID ] !== undefined ) {
                _participants[ nodeID ].instructor = Boolean( propertyValue );    
            }            
            break;   
    }
}
...
vwf_view.firedEvent = function( nodeID, eventName, eventParameters ) {
    ...
    if ( nodeID === appID ) {
        ...
        switch ( eventName ) {
            ...
            case "drawingObjectCreated":
                vwf_view.kernel.callMethod( appID, "recordUserAction", [ currentUserAction, eventParameters[ 0 ] ] );
                break;
        }        
    }
    return eventHandled;
...
( function init () {
    ...
    // show login dialog and call the method below
    vwf_view.kernel.callMethod( appID, "createParticipant", [ vwf_view.kernel.moniker(), "Sgt. Hulka", true ] );
...
function clearAll( user ) {
    clearUserLines( user, undefined );
    clearUserShapes( user, undefined  );
    clearUserUnits( user, undefined  );
    clearBackground();
}

function clearUserShapes( user, layer ){
    var userName = user || vwf_view.kernel.client();
    vwf_view.kernel.callMethod( appID, "clearObjects", [ userName , layer, "shapes" ] );    
}

function clearUserLines( user, layer ){
    var userName = user || vwf_view.kernel.client();
    vwf_view.kernel.callMethod( appID, "clearObjects", [ userName, layer, "lines" ] );    
}

function clearUserUnits( user, layer ){
    var userName = user || vwf_view.kernel.client();
    vwf_view.kernel.callMethod( appID, "clearObjects", [ userName, layer, "units" ] );    
}
...
function newScenario() {
     ...
     // Remove all of the current lines, shapes
     // units, and reset the map
     clearAll( vwf_view.kernel.client() || vwf_view.kernel.moniker() );
...
function loadMap( urlPath ) {
    ...
    if ( require("vwf/utility").validPropertyValue( urlPath ) ) {
...
function resetMap( mapID ) {
    ...
    var stageID = vwf_view.kernel.find( undefined, "/" )[ 0 ]; 
    if ( stageID ) {
        vwf_view.kernel.setProperty( stageID, "position", [ 0, 0 ] );
        vwf_view.kernel.setProperty( stageID, "x", 0 );            
        vwf_view.kernel.setProperty( stageID, "y", 0 ); 
    }

    var backgroundID = vwf_view.kernel.find( undefined, "/background" )[ 0 ]; 
    if ( backgroundID !== undefined ) {
        vwf_view.kernel.setProperty( backgroundID, "position", [ 0, 0 ] );
        vwf_view.kernel.setProperty( backgroundID, "x", 0 );            
        vwf_view.kernel.setProperty( backgroundID, "y", 0 ); 
    } 

    if ( mapID !== undefined ) {
        vwf_view.kernel.setProperty( mapID, "position", [ 0, 0 ] );
        vwf_view.kernel.setProperty( mapID, "x", 0 );            
        vwf_view.kernel.setProperty( mapID, "y", 0 );
    }     
}
...
function createNewLayerFromRequestor() {
    ...
    vwf_view.kernel.callMethod( appID, "createLayer", [ vwf_view.kernel.moniker(), layerName ] );
...
function addLayerDropDown( kineticNode, clientID ) {
    var layerDropDown = $( '#layer-drop-down' );


    if ( layerDropDown ) {
        var listitem = $( '#' + clientID );
        var clientVisibleName = _participants[ clientID ] ? _participants[ clientID ].visibleName : clientID;
        if ( listitem === undefined ) {
            listitem = $( '<li><a id="'+clientID+'">'+clientVisibleName+'</a></li>' ).appendTo( layerDropDown );            
        }

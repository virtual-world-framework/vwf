...
function GetExtension( path ) {
    if ( path.match( /\.vwf$/ ) ) {
        for ( var index = 0; index < template_extensions.length; index++ ) {
            if ( helpers.IsFile( helpers.JoinPath( process.env.HOME + "/.vwf/public/", path + template_extensions[ index ] ) ) ) {
                return template_extensions[ index ];
            }
        }
    }
    else if ( path.match( /\.(dae|unity3d)$/ ) ) {
        if ( helpers.IsFile( helpers.JoinPath( process.env.HOME + "/.vwf/public/", path ) ) ) {
            return "";
        }
    }
    return undefined;
...
function Process( updatedURL ) {
    ...
    while ( ( segments.length > 0 ) && ( helpers.IsDirectory( helpers.JoinPath( process.env.HOME + "/.vwf/public" + result[ 'public_path' ], segments[ 0 ] ) ) ) ) {

...
var templateExtensions = [ "", ".yaml", ".json" ];


function GetExtension( path ) {
    if ( path.match( /\.vwf$/ ) ) {
        for ( var index = 0; index < templateExtensions.length; index++ ) {
            if ( helpers.IsFile( helpers.JoinPath( "./public/", path + templateExtensions[ index ] ) ) ) {
                return templateExtensions[ index ];
            }
        }
    }
    else if ( path.match( /\.(dae|unity3d)$/ ) ) {
        if ( helpers.IsFile( helpers.JoinPath( "./public/", path ) ) ) {
            return "";
        }
    }
    return undefined;
...
function Process( updatedURL ) {
    ...
    while ( ( segments.length > 0 ) && ( helpers.IsDirectory( helpers.JoinPath( "./public" + result[ 'public_path' ], segments[ 0 ] ) ) ) ) {

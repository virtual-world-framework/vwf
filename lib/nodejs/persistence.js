...
function HandlePersistenceUpload( request, response, parsedRequest, segments ) {
    if ( segments.length >= 2 ) {
        ...
        for (var i = 1; i < segments.length; i++) {
            var relPath = libpath.join( relPath, segments[i] );
        }
        CreateDirectory( relPath );
        form.on('progress', function(bytesReceived, bytesExpected) {
            var percent_complete = (bytesReceived / bytesExpected) * 100;
            // report progress of the upload
            console.log("PercentComplete: " + percent_complete);
        }).on('error', function(err) {
            console.log("Error parsing file" + err);
            return false;
        }).on('fileBegin', function(name, file) {
            // redirect file to the specified path
            file.path = helpers.JoinPath( './documents', relPath, file.name);
        });
    ...
    if ( parsedRequest[ 'private_path' ] ) {
        ...
        if ( segments.length > 0 ) {
            switch ( segments[ 0 ] ) {
                ...
                case "upload":
                    if ( ( request.method == "POST" ) && ( segments.length > 1 ) ) {
                        return HandlePersistenceUpload( request, response, parsedRequest, segments );
                    }
            }
        }
    }

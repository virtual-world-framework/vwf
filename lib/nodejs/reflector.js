...
function OnConnection( socket ) {
    ...
    var clients = global.instances[ namespace ].clients;
    clients[ socket.id ] = socket;	 

    socket.pending = true;
    socket.pendingList = [ ];
    socket.isDevice = ( processedURL[ 'private_path' ] === "device" );
    ...
    if ( Object.keys( clients ).length == 1 ) {
        ...
        if ( saveObject ) {
            ...
            socket.emit( 'message', { action: "createNode", parameters: [ "http://vwf.example.com/devices.vwf" ], time: global.instances[ namespace ].getTime( ) } );
    ...
    socket.on( 'message', function ( msg ) {
        ...
        for ( var i in global.instances[ namespace ].clients ) {
            ...
            if( message.result ) {
                //if the message was get state,
                //then fire all the pending messages after firing the setState
                ...
                // If the message was a createChild for a device, only send the response to the device client
                else if ( client.isDevice && message.parameters[0] == "http-vwf-example-com-devices-vwf" &&
                     message.parameters[1] == client.id && message.action == "createChild" ) {
                    client.deviceNode = message.result;
                    client.emit( 'message', message );
                }
            }
            //just a regular message, so push if the client is pending a load, otherwise just send it.
            else if ( client.pending === true ) {
                client.pendingList.push( message );
            }
            else {
                if ( !client.isDevice || (client.deviceNode && message.node === client.deviceNode) ) {
                    client.emit( 'message', message );
                }
            }
        }

...
function GetExtension( path ) {
    if ( path.match( /\.vwf$/ ) ) {
        for ( var index = 0; index < template_extensions.length; index++ ) {
            if ( helpers.IsFile( helpers.JoinPath( "./public/", path + template_extensions[ index ] ) ) ) {
...
function Process( updatedURL ) {
    var result = { 'public_path': "/", 'application': undefined, 'instance': undefined, 'private_path': undefined, 'valid': false };

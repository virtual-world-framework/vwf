...
var create = function( applicationPath ) {
	...
	fs.mkdirpSync( applicationPath );
        ...
        if ( vwfPath != undefined && fs.existsSync( path.join( vwfPath, "support/cli/sample_app/" ) ) ) {
            fs.copySync( path.join( vwfPath, "support/cli/sample_app/" ), applicationPath );
        } else if ( fs.existsSync( path.join( process.cwd(), "support/cli/sample_app/" ) ) ) {

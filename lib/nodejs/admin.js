...
function ServeTimeStateJSON( request, response, instanceHash ) {
    var instanceState = { "time": global.instances[ instanceHash ].time, "rate": global.instances[ instanceHash ].rate, "playing": global.instances[ instanceHash ].playing, "paused": global.instances[ instanceHash ].paused, "stopped": global.instances[ instanceHash ].stopped };
    ...
    if ( ( instanceHash ) && ( request.method == 'GET' ) ) {
        if ( queries ) {
            ...
            if ( ( floatRate ) && ( floatRate > 0.0 ) ) {
                ...
                global.instances[ instanceHash ].playing = true;
                global.instances[ instanceHash ].paused = false;
                global.instances[ instanceHash ].stopped = false;
            }
        }
...
function HandleAdminPause( request, response, instanceHash ) {
    if ( instanceHash ) {
        if ( request.method == 'POST' ) {
            global.instances[ instanceHash ].playing = false;
            global.instances[ instanceHash ].paused = true;
            global.instances[ instanceHash ].stopped = false;
            ServeTimeStateJSON( request, response, instanceHash );
            return true;
        }
...
function HandleAdminStop( request, response, instanceHash ) {
    if ( instanceHash ) {
        if ( request.method == 'POST' ) {
            global.instances[ instanceHash ].playing = false;
            global.instances[ instanceHash ].paused = false;
            global.instances[ instanceHash ].stopped = true;
            global.instances[ instanceHash ].time = 0.0;
            ServeTimeStateJSON( request, response, instanceHash );
            return true;
        }
    }
...
function HandleAdminFiles( request, response, parsedRequest ) {
    if ( ( parsedRequest[ 'public_path' ] ) && ( request.method == "GET" ) ) {
        var filenames = fs.readdirSync( helpers.JoinPath("./public/", parsedRequest[ 'public_path' ] ).replace( /\//g, libpath.sep ) );
        var data = [];
        for ( var i in filenames ) {
            if ( filenames[ i ].match( /\.[jJ][sS][oO][nN]$/ ) ) {
                var filedata = {};
                var filedetails = fs.statSync( helpers.JoinPath( "./public/", parsedRequest[ 'public_path' ], filenames[ i ] ).replace( /\//g, libpath.sep ) );
                if ( filedetails.helpers.IsFile( ) ) {
...
function HandleAdminConfig( request, response, parsedRequest ) {
    if ( ( parsedRequest[ 'public_path' ] ) && ( parsedRequest[ 'application' ] ) && ( request.method == "GET" ) ) {
        var filenameRoot = helpers.JoinPath( "./public/", parsedRequest[ 'public_path' ], parsedRequest[ 'application' ] );
        ...
        if ( helpers.IsFile( filenameRoot + ".config.json" ) ) {
    ...
    if ( parsedRequest[ 'private_path' ] ) {
        ...
        if ( ( segments.length > 0 ) && ( segments[ 0 ] == "admin" ) ) {
            if ( segments.length == 1 ) {
                servehandler.File( request, response, "./support/client/admin.html" );
                return true;
            }

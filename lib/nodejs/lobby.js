...
app.post( "/scenarios", function( req, res, next ) {
  ...
  var scenarioFilename2 =
    "documents/ITDG/" + req.body.name.replace( /[^0-9A-Za-z]+/g, "-" ) + "/saveState_" + ( +new Date() ) + ".vwf.json";
...
app.post( "/sessions", function( req, res, next ) {
  ...
  var sessionFilename2 =
    "documents/ITDG/" + "class_" + req.body.name.replace( /[^0-9A-Za-z]+/g, "-" ) +
      ...
      "_Plt" + req.body.platoon +
      "_Unit" + req.body.unit +  // TODO: year
      "/saveState_" + ( +new Date() ) + ".vwf.json";

  mkdirp( "documents/ITDG/" + "class_" + req.body.name.replace( /[^0-9A-Za-z]+/g, "-" ) + "_Co" + req.body.company + "_Plt" + req.body.platoon + "_Unit" + req.body.unit ).
    ...
    then( () => res.status( 201 ).send( { document: { uri: helpers.JoinPath( "/ITDG", helpers.GenerateInstanceID(), "load", "class_" + req.body.name.replace( /[^0-9A-Za-z]+/g, "-" ) + "_Co" + req.body.company + "_Plt" + req.body.platoon + "_Unit" + req.body.unit ) } } ) ).
    catch( error => next( error ) );

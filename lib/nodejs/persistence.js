...
function GenerateInstanceObject( request, public_path, application, instance_id, active ) {
    var result = {};
    result[ "instance_id" ] = instance_id;
    result[ "url" ] = helpers.JoinPath( "http://" + request.headers.host, public_path, application, instance_id );
    result[ "active" ] = active;
    result[ "vwf_info" ] = {};
    result[ "vwf_info" ][ "public_path" ] = public_path;
    result[ "vwf_info" ][ "application" ] = application;
    result[ "vwf_info" ][ "path_to_application" ] = helpers.JoinPath( public_path, application );
    result[ "vwf_info" ][ "instance" ] = instance_id;
    var metadata = storage.GetInstanceMetadata( public_path, application, instance_id );
    if ( metadata ) {
        result[ "metadata" ] = metadata;
    }
    else {
...
function HandleInstancesLoad( request, response, parsedRequest, segments ) {
    if ( ( request.method == "GET" ) && ( segments.length == 1 ) ) {
        var instance_list = storage.ListApplicationInstances( parsedRequest[ 'public_path' ], parsedRequest[ 'application' ] );
        var instance_objects = [ ];
        var active_instance_list = [ ];
        for ( namespace in global.instances ) {
            var namespace_split = namespace.split("/");
            if ( namespace_split.length > 3 ) {
                var active_instance_id = namespace_split.pop();
                var active_instance_application = namespace_split.pop();
                active_instance_public_path = namespace_split.join("/");
                if ( ( parsedRequest[ 'public_path' ] == active_instance_public_path ) && ( parsedRequest[ 'application' ] == active_instance_application ) ) {
                    active_instance_list.push( active_instance_id );
                    if ( instance_list.indexOf( active_instance_id ) == -1 ) {
                        instance_list.push( active_instance_id );
                    }
                }
            }
        }
        ...
        instance_list.sort();
        for ( var index = 0; index < instance_list.length; index++ ) {
            instance_objects.push( GenerateInstanceObject( request, parsedRequest[ 'public_path' ], parsedRequest[ 'application' ], instance_list[ index ], ( active_instance_list.indexOf( instance_list[ index ] ) > -1 ) ) );
        }
        serve.JSON( instance_objects, response, url.parse( request.url, true ) );
        return true;
    }
    return false;
}

...
// Given a parsedRequest, attempt to load the appropriate application config file.
// Return the Persistence.enabled value from the file, if present.
// Default to false, otherwise.
function EnabledForApplication( parsedRequest ) {
    var result = false;
    var filenameRoot = undefined;
    if ( ( parsedRequest[ 'public_path' ] ) && ( parsedRequest[ 'application' ] ) ) {
        filenameRoot = helpers.JoinPath( "./public/", parsedRequest[ 'public_path' ], parsedRequest[ 'application' ] );
        if ( helpers.IsFile(  filenameRoot + ".config.yaml" ) ) {
            var data = fs.readFileSync( ( filenameRoot + ".config.yaml" ).replace( /\//g, libpath.sep ), "utf8" );
            if ( data ) {
                var file = data.replace( /^\uFEFF/, '' );
                try {
                    var deYAML = YAML.load( file );
                    if ( deYAML.persistence ) {
                        if ( typeof( deYAML.persistence.enabled ) == "boolean" ) {
                            result = deYAML.persistence.enabled
                        }
                    }
                }
                catch( e ) {
            ...
            var data = fs.readFileSync( ( filenameRoot + ".config.json" ).replace( /\//g, libpath.sep ), "utf8" );
            if ( data ) {
                try {
                    var deJSON = JSON.parse( data );
                    if ( deJSON.persistence ) {
                        if ( typeof( deJSON.persistence.enabled ) == "boolean" ) {
                            result = deJSON.persistence.enabled
                        }
                    }
                }
                catch( e ) {
...
exports.EnabledForApplication = EnabledForApplication;
        ...
        return Promise.join( fs.readFileAsync( path ), fs.statAsync( path ), function( json, stats ) {
          ...
          if ( scenarioKey ) {
            updateScenario( applications, applicationKey, scenarioKey, sessionKey, scenarioProperties, {
              uri: applicationKey + "/" + helpers.GenerateInstanceID() + "/load/" + name + "/",
...
function instances( applications ) {
  ...
  return Promise.map( Object.keys( applications ), function( applicationKey ) {
    ...
    var options = {
      url: "http://localhost:3000/" + applicationKey + "/admin/instances",
      json: true,
      proxy: null,
    };
    ...
    return rp( options ).then( function( instances ) {
      ...
      return Promise.all( Object.keys( instances ).map( function( instance ) {
        ...
        return reflector.Evaluate( instance, applicationKey, expression ).then( function( scenarioProperties ) {

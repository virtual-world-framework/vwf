...
function HandleParsableRequest( request, response ) {
    ...
    if ( ( request.url[ request.url.length - 1 ] != "/" ) && ( parsedRequest[ 'private_path' ] == undefined ) ) {
        ...
        else if ( parsedRequest[ 'instance' ] == undefined && request.headers['accept'].indexOf( "text/html" ) != -1 && helpers.IsFile( helpers.JoinPath( "./public/", request.url ) ) ) {
            return servehandler.File( request, response, helpers.JoinPath( "./public/", request.url ) );
        }
        else {

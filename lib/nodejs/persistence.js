...
function CreateSaveDirectory( applicationPath, saveName, saveRevision ) {
    var applicationSegments = helpers.GenerateSegments( applicationPath );
    var currentDirectory = "./documents";
    while ( applicationSegments.length > 0 ) {
        currentDirectory = helpers.JoinPath( currentDirectory, applicationSegments.shift() );
        if ( ! helpers.IsDirectory( currentDirectory ) ) {
            fs.mkdirSync( currentDirectory );
        }
    }
    currentDirectory = helpers.JoinPath( currentDirectory, saveName );
    if ( ! helpers.IsDirectory( currentDirectory ) ) {
        fs.mkdirSync( currentDirectory );
    }
}
...
// an empty array).
function LookupSaveRevisions( publicPath, saveName ) {
    var result = [ ];
    var directoryName = helpers.JoinPath( "./documents/", publicPath, saveName );
...
// and the applicationPath (/path/to/application). It returns an array of all saves found for that
// application (including separate entries for individual revisions of saves ).
function GetSaveInformation( request, response, applicationPath ) {
    var result = [ ];
    var potentialSaveNames = fs.readdirSync( helpers.JoinPath("./documents", applicationPath ) );
    for ( var index = 0; index < potentialSaveNames.length; index++ ) {
        if ( helpers.IsDirectory( helpers.JoinPath( "./documents", applicationPath, potentialSaveNames[ index ] ) ) ) {
            var revisionList = LookupSaveRevisions( applicationPath, potentialSaveNames[ index ] );
            var latestsave = true;
            revisionList.sort();
            while ( revisionList.length > 0 ) {
                var newEntry = {};
                newEntry[ 'applicationpath' ] = applicationPath;
                newEntry[ 'savename' ] = potentialSaveNames[ index ];
                newEntry[ 'revision' ] = revisionList.pop().toString();
                newEntry[ 'latestsave' ] = latestsave;
                if ( latestsave ) {
                    newEntry[ 'url' ] = helpers.JoinPath( "http://" + request.headers.host, applicationPath, "load", potentialSaveNames[ index ] + "/" );
                }
                else {
                    newEntry[ 'url' ] = helpers.JoinPath(  "http://" + request.headers.host, applicationPath, "load", potentialSaveNames[ index ] + "/", newEntry[ 'revision' ] + "/" );
                }
                latestsave = false;
...
function GetSaveInformationRecursive( request, response, applicationPath ) {
    var result = [];
    var subDirectories = fs.readdirSync( helpers.JoinPath("./documents", applicationPath ) );
    for ( var index = 0; index < subDirectories.length; index++ ) {
        if ( helpers.IsDirectory( helpers.JoinPath( "./documents", applicationPath, subDirectories[ index ] ) ) ) {
            result = result.concat( GetSaveInformationRecursive( request, response, helpers.JoinPath( applicationPath, subDirectories[ index ] ) ) );
        }
    }
    result = result.concat( GetSaveInformation( request, response, applicationPath ) );
    return result;
}
...
// GenerateSaveObject is a helper function for HandleSavesLoad.
// It generates an object containing the pertinent information to describe a save.
function GenerateSaveObject( request, publicPath, application, instance, saveName ) {
    var result = {};
    result[ "name" ] = saveName;
    result[ "url" ] = helpers.JoinPath( "http://" + request.headers.host, publicPath, application, instance, "saves", saveName );
    result[ "vwf_info" ] = {};
    result[ "vwf_info" ][ "public_path" ] = publicPath;
    result[ "vwf_info" ][ "application" ] = application;
    result[ "vwf_info" ][ "path_to_application" ] = helpers.JoinPath( publicPath, application );
    result[ "vwf_info" ][ "instance" ] = instance;
    var metadata = storage.GetSaveMetadata( publicPath, application, instance, saveName );
    if ( metadata ) {
...
// HandleSavesLoad attempts to deal with requests to list existing save states.
// If an instance ID was parsed out of the URL, the result will be the JSON
// representation of an array of objects, each describing a save within the
// specified instance of the application.
// If no instance ID was parsed out of the URL, the result will be the JSON
// representation of an object, instance ID's serving as keys paired with
// arrays containing objects, each of which contains information about
// a save within the instance identified by the key.
    ...
    if ( request.method == "GET" ) {
        ...
        if ( segments.length == 1 ) {
            ...
            if ( parsedRequest[ 'instance' ] ) {
                ...
                var saveNameList = storage.ListInstanceSaveStates( parsedRequest[ 'public_path' ], parsedRequest[ 'application' ], parsedRequest[ 'instance' ] );
                var saveObjects = [ ];
                for ( var index = 0; index < saveNameList.length; index++ ) {
                    saveObjects.push( GenerateSaveObject( request, parsedRequest[ 'public_path' ], parsedRequest[ 'application' ], parsedRequest[ 'instance' ], saveNameList[ index ] ) );
                }
                serve.JSON( saveObjects, response, url.parse( request.url, true ) );
                return true;
            }
            else {
                //Listing for all save states for the application, not limited to one specific instance.
                var saveHash = storage.ListApplicationSaveStates( parsedRequest[ 'public_path' ], parsedRequest[ 'application' ] );
                var instanceHash = { };
                for ( instanceId in saveHash ) {
                    var saveObjects = [ ];
                    for ( var index = 0; index < saveHash[ instanceId ].length; index++ ) {
                        saveObjects.push( GenerateSaveObject( request, parsedRequest[ 'public_path' ], parsedRequest[ 'application' ], instanceId, saveHash[ instanceId ][ index ] ) );
                    }
                    instanceHash[ instanceId ] = saveObjects;
                }
                serve.JSON( instanceHash, response, url.parse( request.url, true ) );
                return true;
            }
...
function GenerateInstanceObject( request, publicPath, application, instanceId, active ) {
    var result = {};
    result[ "instance_id" ] = instanceId;
    result[ "url" ] = helpers.JoinPath( "http://" + request.headers.host, publicPath, application, instanceId );
    result[ "active" ] = active;
    result[ "vwf_info" ] = {};
    result[ "vwf_info" ][ "public_path" ] = publicPath;
    result[ "vwf_info" ][ "application" ] = application;
    result[ "vwf_info" ][ "path_to_application" ] = helpers.JoinPath( publicPath, application );
    result[ "vwf_info" ][ "instance" ] = instanceId;
    var metadata = storage.GetInstanceMetadata( publicPath, application, instanceId );
    if ( metadata ) {
        result[ "metadata" ] = metadata;
    }
    else {
...
function HandleInstancesLoad( request, response, parsedRequest, segments ) {
    if ( ( request.method == "GET" ) && ( segments.length == 1 ) ) {
        var instanceList = storage.ListApplicationInstances( parsedRequest[ 'public_path' ], parsedRequest[ 'application' ] );
        var instanceObjects = [ ];
        var activeInstanceList = [ ];
        for ( namespace in global.instances ) {
            var namespaceSplit = namespace.split("/");
            if ( namespaceSplit.length > 3 ) {
                var activeInstanceId = namespaceSplit.pop();
                var activeInstanceApplication = namespaceSplit.pop();
                var activeInstancePublicPath = namespaceSplit.join("/");
                if ( ( parsedRequest[ 'public_path' ] == activeInstancePublicPath ) && ( parsedRequest[ 'application' ] == activeInstanceApplication ) ) {
                    activeInstanceList.push( activeInstanceId );
                    if ( instanceList.indexOf( activeInstanceId ) == -1 ) {
                        instanceList.push( activeInstanceId );
                    }
                }
            }
        }
        ...
        instanceList.sort();
        for ( var index = 0; index < instanceList.length; index++ ) {
            instanceObjects.push( GenerateInstanceObject( request, parsedRequest[ 'public_path' ], parsedRequest[ 'application' ], instanceList[ index ], ( activeInstanceList.indexOf( instanceList[ index ] ) > -1 ) ) );
        }
        serve.JSON( instanceObjects, response, url.parse( request.url, true ) );
        return true;
    }
    return false;
}
    ...
    if ( parsedRequest[ 'private_path' ] ) {
        ...
        if ( segments.length > 0 ) {
            switch ( segments[ 0 ] ) {
                ...
                case "instances":
                    return HandleInstancesLoad( request, response, parsedRequest, segments );
            }
        }
    }
    return false;

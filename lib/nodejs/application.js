...
function Serve( request, response, parsedRequest ) {
    ...
    if ( servehandler.File( request, response, helpers.JoinPath( process.env.HOME + "/.vwf/support/client/lib/", fileName ) ) ) {
        return true;
    }
    ...
    if ( servehandler.File( request, response, helpers.JoinPath( process.env.HOME + "/.vwf/public/", parsedRequest[ 'public_path' ], parsedRequest[ 'private_path' ] ) ) ) {
        return true;
    }
    ...
    if ( servehandler.Component( request, response, helpers.JoinPath( process.env.HOME + "/.vwf/public/", parsedRequest[ 'public_path' ], parsedRequest[ 'private_path' ] ) ) ) {
        return true;
    }

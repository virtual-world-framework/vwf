      ...
      if ( ! revision ) {
        ...
        return Promise.join( fs.readFileAsync( path ), fs.statAsync( path ), function( json, stats ) {
          ...
          var applicationNode = ( ( state || {} ).nodes || [] ).filter( function( node ) {
            return node.patches === application;
          } )[ 0 ];
          ...
          var scenarioProperties = ( ( ( applicationNode || {} ).children || {} ).scenarioDescription || {} ).properties || {};

          var scenarioTitle = scenarioProperties.scenarioTitle;
          var scenarioClassroom = scenarioProperties.classroom || {};

          if ( scenarioTitle ) {
            updateScenario( applications, application, scenarioTitle, scenarioClassroom, {
              uri: application + "/" + helpers.GenerateInstanceID() + "/load/" + name + "/",
...
function i( applications ) {
  ...
  return Promise.map( Object.keys( applications ), function( application ) {
    ...
    return rp( options ).then( function( instances ) {
      ...
      var expression = "this.scenarioDescription && this.scenarioDescription.properties";

      return Promise.all( Object.keys( instances ).map( function( instance ) {

        return reflector.Evaluate( instance, application, expression ).then( function( scenarioProperties ) {
          ...
          if ( scenarioTitle ) {
            updateScenario( applications, application, scenarioTitle, scenarioClassroom, undefined, instance );
          }
...
function updateScenario( applications, application, scenarioTitle, scenarioClassroom, document, instance ) {

  var scenario = applications[ application ][ scenarioTitle ] = applications[ application ][ scenarioTitle ] || {
    application:
      application,
    scenario: {
      state: {
  ...
  if ( ! scenarioClassroom.company && ! scenarioClassroom.platoon && ! scenarioClassroom.unit ) {
    ...
    var matchedSession = scenario.sessions.reduce( function( matchedSession, session ) {
      ...
      var sessionClassroom = session.state.classroom;

      if ( sessionClassroom.company === scenarioClassroom.company && sessionClassroom.platoon === scenarioClassroom.platoon &&
          sessionClassroom.unit === scenarioClassroom.unit ) {
        if ( document ) {
    ...
    if ( ! matchedSession ) {
      ...
      var session = {
        state: {
          classroom: {
            company: scenarioClassroom.company,
            platoon: scenarioClassroom.platoon,
            unit: scenarioClassroom.unit } },
      };

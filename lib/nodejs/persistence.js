...
function GenerateInstanceObject( request, public_path, application, instance_id ) {
    var result = {};
...
function HandleInstancesLoad( request, response, parsedRequest, segments ) {
    if ( ( request.method == "GET" ) && ( segments.length == 1 ) ) {
        ...
        for ( var index = 0; index < instance_list.length; index++ ) {
            instance_objects.push( GenerateInstanceObject( request, parsedRequest[ 'public_path' ], parsedRequest[ 'application' ], instance_list[ index ] ) );
        }

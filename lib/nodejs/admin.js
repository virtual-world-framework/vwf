    ...
    if ( ( parsedRequest[ 'public_path' ] ) && ( request.method == "GET" ) ) {
        ...
        for ( var i in filenames ) {
            if ( filenames[ i ].match( /\.[jJ][sS][oO][nN]$/ ) ) {
                var filedata = {};
                var filedetails = fs.statSync( helpers.JoinPath( process.env.HOME + "/.vwf/public/", parsedRequest[ 'public_path' ], filenames[ i ] ).replace( /\//g, libpath.sep ) );
                if ( filedetails.helpers.IsFile( ) ) {
...
function HandleAdminChrome( request, response, parsedRequest ) {
    ...
    if ( publicPath && appName && ( request.method == "GET" ) ) {
        var fileBasePath = helpers.JoinPath( process.env.HOME + "/.vwf/public/", publicPath, appName );
    ...
    if ( parsedRequest[ 'private_path' ] ) {
        ...
        if ( ( segments.length > 0 ) && ( segments[ 0 ] == "admin" ) ) {
            if ( segments.length == 1 ) {
                servehandler.File( request, response, process.env.HOME + "/.vwf/support/client/admin.html" );
                return true;
            }
